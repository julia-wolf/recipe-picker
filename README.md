...
Heroku Basic (stays awake) and Postgres Mini (one `DATABASE_URL`, 20 connections). Rails 8 wanted extra databases for cache, jobs, and cable; this app has no jobs or WebSockets, so production uses in-process memory and one Postgres. Do not add Redis.

Set `DATASET_URL` and `RAILS_MASTER_KEY` on the app. Do not commit `config/master.key`. Import is a one-off after the first deploy — not a `release:` task (the gzip wipe/reload can time out).
...
heroku config:set DATASET_URL="https://pennylane-interviewing-assets-20220328.s3.eu-west-1.amazonaws.com/recipes-en.json.gz"
git push heroku main
heroku run bin/rails recipes:import
```

Destroy the app and addon after the interview. List prices are prorated hourly.
